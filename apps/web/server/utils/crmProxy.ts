...
export async function proxyCrmRequest(event: H3Event, path: string) {
  const config = useRuntimeConfig(event)
  const target = new URL(path, config.crmApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'CRM proxy request failed',
