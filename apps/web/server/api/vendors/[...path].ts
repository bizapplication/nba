...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/vendors/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Vendor proxy request failed',
