...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/departments/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Department proxy request failed',
