...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/finance/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Finance proxy request failed',
