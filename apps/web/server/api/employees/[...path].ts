...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/employees/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Employee proxy request failed',
