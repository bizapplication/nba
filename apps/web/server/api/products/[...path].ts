...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/products/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Product proxy request failed',
