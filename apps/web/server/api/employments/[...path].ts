...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/employments/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Employment proxy request failed',
