...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/positions/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Position proxy request failed',
