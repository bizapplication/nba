...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/customers/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Customer proxy request failed',
