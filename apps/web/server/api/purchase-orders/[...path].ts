...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/purchase-orders/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Purchase order proxy request failed',
