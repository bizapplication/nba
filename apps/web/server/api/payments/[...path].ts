...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/payments/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Payment proxy request failed',
