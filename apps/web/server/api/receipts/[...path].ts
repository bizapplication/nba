...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/receipts/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Receipt proxy request failed',
