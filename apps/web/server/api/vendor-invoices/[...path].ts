...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/vendor-invoices/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Vendor invoice proxy request failed',
