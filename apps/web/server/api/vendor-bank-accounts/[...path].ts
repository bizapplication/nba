...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/vendor-bank-accounts/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Vendor bank account proxy request failed',
