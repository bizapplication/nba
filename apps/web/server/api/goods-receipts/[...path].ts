...
export default defineEventHandler(async (event) => {
  ...
  const target = new URL(`/api/goods-receipts/${path}`, config.financeApiBase)
  ...
  try {
    ...
    throw createError({
      ...
      statusMessage: fetchError.response?._data?.message || fetchError.response?.statusText || 'Goods receipt proxy request failed',
